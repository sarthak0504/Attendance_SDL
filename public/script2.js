document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('sendButton').addEventListener('click', function() {
        const enrollments = Array.from(document.querySelectorAll('#studentTable tbody tr'))
            .map(row => row.children[0].textContent); // Adjusted to get the Enrollement value from the first column
        ...
        .catch(error => {
            console.error('Error:', error);
            alert('Failed to send audio messages. Please try again later.');
        });
    });
